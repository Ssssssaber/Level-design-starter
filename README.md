...
2. Откройте папку с проектом с помощью Unity Editor (разрабатывалось и тестировалось только на версии 6000.3.7f1)
3. Откройте Control Scene
4. Добавьте в Build settings сцены Control, Menu и Action (если их там еще нет)
5. Можно запускать и пробовать проект

---

## Дополнительные материалы
- **Документация по реализации**: См. папку `Docs/Implementation` для деталей по коду и логике (автоматически сгенерировано, могут быть неточности. Подходит для базового ознакомления с кодом системам, если вам интересно).
- **Примеры головоломок**: См. `PUZZLES` объект в ActionScene сцене для взаимодействия ловушек, факелов, врагов и дверей.

## Сообщение о багах и ваши предложения (Issues)
Если у вас есть предложения или были обнаружены баги, создавайте в Issue в репозитории. Буду стараться на них отвечать.

### Примеры Issue

**Плохо (Сурово - чтобы разраб вообще офигел и не понял, че далать: как воспроизвести ошибку, что исправить. Это придает разработчику проекта челленджа и точно стимулирует уничтожение большего количества нервных клеток разраба, что сокращает продолжительности его жизни и, возможно, провоцирует его на негативный ответ с использованием ненормативной лексики или прямое игнорирование таких Issue. Но зато это отличный способ высказать все, что вы думаете о разработчике, ведь вы тоже страдаете, роясь в его бездарном проекте - лаба сама себя не закроет):**
```
Персонажу игрока в игре ультра впадлу ходить и вообще реагировать на мои действия и инпуты. Разраб, ты бездарь. Теперь ты должен испечь мне 10 ирисково-коричных пирогов. 
```

**Хорошо (Часть с бездарностью можете перенести из прошлого примера, чтобы совместить конструктивную оценку и разрядку вашего эмоционального состояния - не стоит копить в себе гнев. Забота о себе превыше всего!):**
```
Ошибка при прыжке
...
Разраб бездарность. Выглядит так, будто это его первый проект, который он сделал за 1 час по туториалам
```
